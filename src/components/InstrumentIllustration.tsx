import { type Product, getBrand } from "@/lib/data";
...
function DisplayContent({ category }: { category: string }) {
  switch (category) {
    ...
    default:
      return (
        <g>
          <path
            d="M 55 75 L 70 75 L 75 62 L 85 90 L 95 75 L 110 75 L 115 66 L 125 82 L 145 75"
            stroke="#0a4da2"
            strokeWidth="2"
            fill="none"
            strokeLinecap="round"
...
export default function InstrumentIllustration({ product, className = "" }: Props) {
  ...
  const isScope =
    ...
    product.category === "rf-microondas";
      ...
      {isScope ? (
        <g>
          ...
          <rect
            ...
            stroke={bodyStroke}
          />
          <DisplayContent category={product.category} />
          <g>
        ...
        <g>
          ...
          <text
            ...
            letterSpacing="0.8"
          >
            {brand?.name?.toUpperCase() ?? "LIDERTESTES"}
          </text>
        </g>
      )}
